 ...
 * 
 */
class ShitcoinsVolatilityStrategy implements SwingTradingStrategyInterface
{
    private const MIN_AMOUNT_OF_MONEY = 20;

    private const AMOUNT_OF_PREVIOUS_CANDLESTICKS = 720;
    private const AMOUNT_OF_NEXT_CANDLESTICKS = 200;
    private const MIN_VOLUME = 100_000;
    private const MAX_AMOUNT_TRADES_PER_DAY = 10;

    private const MIN_AMOUNT_OF_CANDLESTICKS = 200;

    private const MIN_VOLATILITY = 0.1;

    private const TARGET_PERCENTAGE = 0.1;    // 20 percent is default 
    private const STOP_LOSS_PERCENTAGE = 0.1;    // 25 percent is default 

    private const MAX_PRICE = 1;
    private const ATR_LIMIT = 0.1;
    ...
    private function isSecurityEligibleForTrading(array $lastCandleSticks, Security $security, string $position) : bool
    {
        ...
        if($closePrice > $sma200 && $closePrice > 0 && $atr14 > self::MIN_VOLATILITY)
        {
