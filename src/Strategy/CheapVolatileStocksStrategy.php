 ...
 * 
 */
class CheapVolatileStocksStrategy implements SwingTradingStrategyInterface
{
    ...
    private const MIN_ATR_PERCENTAGE = 0.1;
    ...
    private function isSecurityEligibleForTrading(array $lastCandleSticks, $tradingDate) : bool
    {
        ...
        if($closePrice < self::MIN_PRICE || $volume < self::MIN_VOLUME || $closePrice > self::MAX_PRICE)
            return false;
          ...
          )
        {
            ...
            $this->addTradingDataInformation(BaseConstants::TRADE_STOP_LOSS_PRICE, $closePrice * 0.5);
    ...
    private function getProfit(Security $security, $stopLoss, $sharesAmount, DateTime $tradingDate, float $enterPrice) : float 
    {
        ...
        $nextCandleSticks = $security->getNextNCandleSticks($tradingDate, 10);      // TODO: that's the biggest change
        ...
        foreach ($nextCandleSticks as $candleStick) {
            ...
            if($closePrice > $takeProfit && $closePrice < $sma50)        // it must be a winner
            {
                // so that means that you should leave your position 30 minutes before market close let's say
