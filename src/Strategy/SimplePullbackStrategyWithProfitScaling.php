 ...
 */
class SimplePullbackStrategyWithProfitScaling implements SwingTradingStrategyInterface
{
    ...
    private function getProfit(Security $security, $stopLoss, $sharesAmount, DateTime $tradingDate, float $spread, float $enterPrice) : float 
    {
        ...
        foreach ($nextCandleSticks as $candleStick) {
            ...
            if($closePrice <= $stopLoss && !$target1)
            {
            ...
            if($closePrice <= $stopLoss && $target1)
            {
                ...
                return $profit + ($stopLoss - $spread) * (float)($sharesAmount / 2);
            }


            if($closePrice < $previousCandleStick->getClosePrice() && $closePrice > $enterPrice && !$target1)
            {
                $target1 = true;
                $profit = ($closePrice - $spread) * (float)($sharesAmount / 2);
                $target1ClosePrice = $closePrice;
            }

            if($closePrice < $previousCandleStick->getClosePrice() && $closePrice > $enterPrice && $target1 && $closePrice > $target1ClosePrice)
            {
                // so that means that you should leave your position 30 minutes before market close let's say
                $this->addTradingDataInformation(BaseConstants::TRADE_EXIT_PRICE, $closePrice - $spread);
                $this->addTradingDataInformation(BaseConstants::EXIT_DATE, $exitDate->format('Y-m-d'));

                $riskRewardRatio = (($closePrice + $target1ClosePrice) / 2 - $enterPrice - 2 * $spread) / ($enterPrice - $stopLoss) ;
                $this->addTradingDataInformation(BaseConstants::TRADE_RISK_REWARD, $riskRewardRatio);


                return $profit + ($closePrice - $spread) * (float)($sharesAmount / 2);
            }
