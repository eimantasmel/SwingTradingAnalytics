 ...
 * 
 */
class MacdStrategy implements SwingTradingStrategyInterface
{
    ...
    private const RISK_REWARD = 1.6;    
    private const CHOP_THRESHOLD = 30;

    private const EARLIER_MACD_DAYS_AMOUNT = 5;
    private const MIN_MACD_LINE_CHANGE_PERCENGATE = 0.25;
    ...
    private function isSecurityEligibleForTrading(array $lastCandleSticks, Security $security) : bool
    {
        ...
        if(
            $chopIndex < self::CHOP_THRESHOLD        // If there's will be only few trades then delete this line
            && $latestMacdData['macdLine'] > $latestMacdData['signalLine']
            && $oneDayEarlierMacdData['macdLine'] < $oneDayEarlierMacdData['signalLine']
            && abs($latestMacdData['macdLine'] / $nDaysEarlierMacdData['macdLine'] - 1) >= self::MIN_MACD_LINE_CHANGE_PERCENGATE 
          )
        {
            ...
            $stopLoss = $previousCandleStick->getLowestPrice();
