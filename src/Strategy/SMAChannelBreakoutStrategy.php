...
class SMAChannelBreakoutStrategy implements SwingTradingStrategyInterface
{
    ...
    private const MIN_SMA_CHANNEL_LENGTH = 20;
                                     ...
                                     ) : array
    {
        ...
        while($startDate < $endDate)
        {
            ...
            $randomDateInterval = (int)mt_rand(1, 3);
    ...
    private function isSecurityEligibleForTrading(array $lastCandleSticks, $tradingDate, $security) : bool
    {
        ...
        if($closePrice > $sma20 
            && $closePrice > $sma50
            && $this->isSMAChannelValid($security, $lastCandleSticks, $tradingDate)
          )
        {
            $this->addTradingDataInformation(BaseConstants::TRADE_POSITION, "Long");
            $this->addTradingDataInformation(BaseConstants::TRADE_STOP_LOSS_PRICE, max($sma20, $sma50) - 2 * $atr14);
    ...
    private function isSMAChannelValid($security, $lastCandleSticks, $tradingDate): bool
    {
        $candleSticks = array_slice($lastCandleSticks, -1 * self::MIN_SMA_CHANNEL_LENGTH);

        foreach ($candleSticks as $candleStick) {
            if($candleStick->getDate()->diff($tradingDate)->days  == 0)
                continue;
            $closePrice = $candleStick->getClosePrice();

            $lastSticks = $security->getLastNCandleSticks($candleStick->getDate(), self::AMOUNT_OF_PREVIOUS_CANDLESTICKS);
            $prices = $this->extractClosingPricesFromCandlesticks($lastCandleSticks);
            $sma50 = $this->technicalIndicatorsService->calculateSMA($prices, 50);
            $sma20 = $this->technicalIndicatorsService->calculateSMA($prices, 20);

            if($closePrice < $sma20 && $closePrice < $sma50)
                return false;
    ...
    private function getProfit(Security $security, $stopLoss, $sharesAmount, DateTime $tradingDate, float $enterPrice) : float 
    {
        ...
        foreach ($nextCandleSticks as $candleStick) {
            ...
            if($closePrice < $sma20)        // it might be a winner
            {
                // so that means that you should leave your position 30 minutes before market close let's say
